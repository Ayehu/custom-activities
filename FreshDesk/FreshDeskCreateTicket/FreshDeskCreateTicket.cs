...
{
    public class FreshDeskCreateTicket : IActivity
    {
        public string fdDomain, apiKey;
        public string status, priority, email, subject, description, custom_fields, filePath;
        ...
        private void MultipartRequest(string apiPath)
        {
            ...
            // Web Request:
            HttpWebRequest wr = (HttpWebRequest)WebRequest.Create("https://" + fdDomain + ".freshdesk.com" + apiPath);
            ...
            using (var rs = wr.GetRequestStream())
            {
                ...
                writeString(rs, status.Split(' ')[0]);
                writeCRLF(rs);

                // Priority:
                writeBoundaryBytes(rs, boundary, false);
                writeContentDispositionFormDataHeader(rs, "priority");
                writeString(rs, priority.Split(' ')[0]);
                writeCRLF(rs);

                // custom_fields
                if (!string.IsNullOrEmpty(custom_fields))
                {
                    var customFields = JsonConvert.DeserializeObject<JObject>(custom_fields);

                    var fields = customFields.Children<JProperty>();

                    foreach (var field in fields)
                    {
                        writeBoundaryBytes(rs, boundary, false);
                        writeContentDispositionFormDataHeader(rs, "custom_fields["+field.Name+"]");
                        writeString(rs, field.Value.ToString());
                        writeCRLF(rs);
                    }
                }
                ...
                {
                    writeBoundaryBytes(rs, boundary, false);
                    writeContentDispositionFileHeader(rs, "attachments[]", filePath.Split('/').Last(), GetMimeType(filePath.Split('.').Last()));
