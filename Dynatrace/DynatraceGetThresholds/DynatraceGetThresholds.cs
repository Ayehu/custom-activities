...
{
    public class DynatraceGetThresholds : IActivity
    {
        #region Private readonly properties

        private readonly string API_REQUEST_URL = "https://{0}.live.dynatrace.com/api/v1/thresholds?filter=ALL";
        ...
        public ICustomActivityResult Execute()
        {
            ...
            if (httpResponse.StatusCode == HttpStatusCode.OK)
            {
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                {
                    ...
                    List<string> logsDataKeys = new List<string>(){ "thresholdId",
                                                                    "threshold",
                                                                    "alertCondition",
                                                                    "samples",
                                                                    "violatingSamples",
                                                                    ...
                                                                    "description",
                                                                    "timeseriesId"};
                    var _thresholds = JToken.Parse(response);

                    var thresholdList = new List<Dictionary<string, string>>();

                    foreach (var _threshold in _thresholds)
                    {
                        var res = ExposeJson(_threshold.ToObject<JObject>(), "");
                        ...
                        thresholdList.Add(data as Dictionary<string, string>);
