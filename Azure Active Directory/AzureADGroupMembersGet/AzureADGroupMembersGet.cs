...
{
    public class AzureADGroupMembersGet : IActivity
    {
        ...
        private string uriBuilderPath
        {
            get
            {
                ...
                {
                    _uriBuilderPath = string.Format("/v1.0/groups/{0}/members", groupId);
                }
                ...
                case HttpStatusCode.OK:
                    {
                        if (string.IsNullOrEmpty(response.Content.ReadAsStringAsync().Result) == false)
                        {
                            ...
                            using (StreamReader sr = new StreamReader(response.Content.ReadAsStreamAsync().Result))
                            {
                                ...
                                foreach(var member in json.Value<JToken>("value"))
                                {
                                    dt.Rows.Add
                                        (
                                        member.Value<string>("id"),
                                        member.Value<string>("@odata.type").Contains("user") ? "User" : "Role",
                                        member.Value<string>("displayName"),
