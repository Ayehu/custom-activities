	...
	public class CustomActivity : IActivity
	{
		...
		public ICustomActivityResult Execute()
		{
			if(formality != "default" && (targetLanguage != "DE" && targetLanguage != "FR" && targetLanguage != "IT" && targetLanguage != "ES"
						...
						&& targetLanguage != "RU"))
			{
				throw new Exception("Target language does not support formality mode \"" + formality + "\". Please use \"default\" setting. Only the following languages support formality levels:\nDE, FR, IT, ES, NL, PL, PT-PT, PT-BR, RU");
			}
			else
			{
