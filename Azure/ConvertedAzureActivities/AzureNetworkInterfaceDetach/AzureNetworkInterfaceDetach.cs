...
{
    class AzureNetworkInterfaceDetach : IActivity
    {
        /// <summary>
        ...
        public ICustomActivityResult Execute()
        {
            if (SetVMState(VMState.Dealllocate))
            {
                ...
                try
                {
                    var response = DetachNetworkInterface();
        ...
        private HttpResponseMessage DetachNetworkInterface()
        {
        ...
        private string GetVirtualMachineNetworkProfile()
        {
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                {
                    var network = networkProfile.Where(x => x["id"].ToString() == interfaceId).First();
                    networkProfile.Remove(network);
                }
