...
{
    public class AzureVMDiskDetach : IActivity
    {
        // <summary>
        ...
        private string uriBuilderPath
        {
            get
            {
                ...
                {
                    _uriBuilderPath = string.Format("/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Compute/virtualMachines/{2}", subscriptionId, resourceGroupName, vmName);
                }
        ...
        private string GetVMDiskToDetach()
        {
            postData = "";
            httpMethod = "GET";
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                JArray disks = (JArray)json["properties"]["storageProfile"]["dataDisks"];
                var diskToRemove = disks.Where(d => d.Value<string>("name") == diskName).FirstOrDefault();
                ...
                return (json["properties"]["storageProfile"]["dataDisks"]).ToString();
            }

            return string.Empty;
        }

        private string GetVMDisks()
        {
            return @"{
                      ""properties"": {
                          ""storageProfile"": {
                          ""dataDisks"":" + GetVMDiskToDetach() + @"
                    }}}";
        }
    }
}
