...
Namespace Ayehu.Sdk.ActivityCreation
    Public Class CustomActivity
        ...
        Public Function Execute() As ICustomActivityResult Implements IActivity.Execute
            ...
            'this will look for the token to be extened with the latest date from the one available.
            ...
            If results.Length = 0 Then Throw New System.Exception("Unable to locate token for user " + AccountName)
            targetguid = results(0).guid

            Dim TokentoUse As String
            Dim newdate As DateTime = DateAdd(DateInterval.Day, MDays, DateTime.Now)

            Dim cmd2 As New SearchTokensCommand
            cmd2.filter = Filter.greater(ListTokenDTO._FILTER_EXPIRES_ON, newdate)
            cmd2.limit = 1
            cmd2.assigned = False
            cmd2.execute()
            results = cmd2.listSecurIDTokens
            If results.Length = 0 Then Throw New System.Exception("Unable to locate available token")
            TokentoUse = results(0).guid


            Dim etk As New ExtendTokenDTO
            etk.extendedTokenGuid = targetguid
            etk.extentionTokenGuid = TokentoUse

            Dim cm4 As New ExtendTokensCommand
            cm4.extTknDTO = New ExtendTokenDTO() {etk}
            cm4.execute()
