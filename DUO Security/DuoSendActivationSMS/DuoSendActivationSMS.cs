...
{
	public class DuoSendActivationSMS : IActivity
	{
		...
		public string secondsValid;
		public int customSecondsValid;

		public ICustomActivityResult Execute()
		{
			var parameters = new Dictionary<string, string>();
			
			if(customSecondsValid == 0)
			{
				parameters.Add("valid_secs", secondsValid);
			}

			if(!String.IsNullOrEmpty(activationMessage))
			{
				parameters.Add("activation_msg", activationMessage);
			}

			if(!String.IsNullOrEmpty(installationMessage))
			{
				parameters.Add("install", "1");
				parameters.Add("installation_msg", installationMessage);
			}

			HttpStatusCode code = HttpStatusCode.BadRequest;
			
			var results = new DuoApi(integrationKey, secretKey, apiHost, "https").ApiCall("POST", "/admin/v1/phones/" + phoneID + "/send_sms_activation", parameters, 0, DateTime.UtcNow, out code);
