...
{
    public class DuoGetUserID : IActivity
    {
        ...
        public ICustomActivityResult Execute()
        {
            ...
            var results = new DuoApi(integrationKey, secretKey, apiHost, "https").ApiCall("GET", "/admin/v1/users", parameters, 0, DateTime.UtcNow, out code);

            JObject jsonResults = JObject.Parse(results);

            JArray users = (JArray)jsonResults["response"];
            ...
            else
            {
                ...
                for(int i = 0; i < userCount; i ++)
                {
                    ...
                    dt.Rows[i]["Username"] = jsonResults["response"][i]["username"].ToString();
                    dt.Rows[i]["User Id"] = jsonResults["response"][i]["user_id"].ToString();
                }
