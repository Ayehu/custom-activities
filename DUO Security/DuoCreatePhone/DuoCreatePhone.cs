...
{
	public class DuoCreatePhone : IActivity
	{
		...
		public int enablePhonePredelay;
		public string phonePredelay;
		public int enablePhonePostdelay;
		public string phonePostdelay;

		public ICustomActivityResult Execute()
		{
			var parameters = new Dictionary<string, string>();
			
			if(enablePhonePredelay == 1)
			{
				parameters.Add("predelay", phonePredelay);
			}

			if(enablePhonePostdelay == 1)
			{
				parameters.Add("postdelay", phonePostdelay);
			}

			if(!String.IsNullOrEmpty(phoneNumber))
			{
				parameters.Add("number", phoneNumber);
			}

			if(!String.IsNullOrEmpty(phoneName))
			{
				parameters.Add("name", phoneName);
			}

			if(!String.IsNullOrEmpty(phoneExtension))
			{
				parameters.Add("extension", phoneExtension);
			}

			if(!String.IsNullOrEmpty(phoneType))
			{
				parameters.Add("type", phoneType);
			}

			if(!String.IsNullOrEmpty(phonePlatform))
			{
				parameters.Add("platform", phonePlatform);
			}

			HttpStatusCode code = HttpStatusCode.BadRequest;
			
			var results = new DuoApi(integrationKey, secretKey, apiHost, "https").ApiCall("POST", "/admin/v1/phones", parameters, 0, DateTime.UtcNow, out code);

			JObject jsonResults = JObject.Parse(results);

			if(jsonResults["stat"].ToString() == "OK")
			{
				return this.GenerateActivityResult(jsonResults["response"]["phone_id"].ToString());
			}
			else
			{
