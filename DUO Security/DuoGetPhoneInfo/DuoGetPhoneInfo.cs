...
{
	public class DuoGetPhoneInfo : IActivity
	{
		...
		public ICustomActivityResult Execute()
		{
			...
			var results = new DuoApi(integrationKey, secretKey, apiHost, "https").ApiCall("GET", "/admin/v1/phones", parameters, 0, DateTime.UtcNow, out code);

			JObject jsonResults = JObject.Parse(results);

			JArray phones = (JArray)jsonResults["response"];
			...
			else
			{
				...
				for(int i = 0; i < phoneCount; i ++)
				{
					...
					JObject phoneDetails = JObject.Parse(jsonResults["response"][i].ToString());

					foreach(JProperty property in phoneDetails.Properties())
					{
				...
				dt.Rows[0]["username"] = jsonResults["response"][0]["users"][0]["username"].ToString();

				dt.Columns.Add("realname");
				dt.Rows[0]["realname"] = jsonResults["response"][0]["users"][0]["realname"].ToString();

				dt.Columns.Add("user_id");
				dt.Rows[0]["user_id"] = jsonResults["response"][0]["users"][0]["user_id"].ToString();
