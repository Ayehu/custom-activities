...
namespace Ayehu.Sdk.ActivityCreation
{
	public class CustomActivity : IActivity
	{
		...
		public ICustomActivityResult Execute()
		{
			...
			else if(registryKey == "HKEY_CURRENT_USER")
			{
				registryKey = "HKCU";
			}
			else if(registryKey == "HKEY_LOCAL_MACHINE")
			{
				registryKey = "HKLM";
			}
			else if(registryKey == "HKEY_USERS")
			{
				registryKey = "HKU";
			}
			else if(registryKey == "HKEY_CURRENT_CONFIG")
			{
				registryKey = "HKCC";
			}

			ScriptCode = "Write-Output 'Executing...' ; Set-ItemProperty '" + registryKey + ":\\" + registryPath + "' -Name '" + itemName + "' -Value '" + itemValue + "' -Type " + itemType + " -Force 2>&1";
			...
			try
			{
				...
				{
					output = ExtractString(ExecuteLocal(ScriptPath, ScriptCode, HasParams, TableAsString), "Result");
				...
				{
					output = ExtractString(ExecuteRemote(PowershellTimeOut, HostName, UserName, Password, PsrPort, UriType, ScriptCode, ScriptPath, HasParams.ToString(), TableAsString), "Result");
