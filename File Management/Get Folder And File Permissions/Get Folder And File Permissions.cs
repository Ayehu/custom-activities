...
namespace Ayehu.Sdk.ActivityCreation
{
	public class CustomActivity : IActivity
	{
		...
		public ICustomActivityResult Execute()
		{
			...
			{
			    $JSON += '{""Identity Reference"": ';
			    $JSON += $Access.IdentityReference.Value | ConvertTo-Json;
			    $JSON += ',';

			    $JSON += '""FileSystemRights"": ""';
			    $JSON += $Access.FileSystemRights.ToString();
			    $JSON += '"",';

			    $JSON += '""IsInherited"": ""';
			    $JSON += $Access.IsInherited.ToString();
			    $JSON += '""},';
			}
			$JSON = $JSON -replace "".{1}$"";
			$JSON += ""]"";
