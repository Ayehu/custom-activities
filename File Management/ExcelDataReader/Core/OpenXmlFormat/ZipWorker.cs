...
namespace ExcelDataReader.Core.OpenXmlFormat
{
    internal partial class ZipWorker : IDisposable
    {
        ...
        public RecordReader GetWorksheetReader(string sheetPath)
        {
            ...
            if (zipEntry != null)
            {
                switch (Path.GetExtension(sheetPath))
                {
                    case ".xml":
                        {
                            return new XmlWorksheetReader(XmlReader.Create(zipEntry.Open(), XmlSettings));
                        }
                    case ".bin":
                        {
                            return new BiffWorksheetReader(zipEntry.Open());
                        }
                    default:
                        return null;
                }
