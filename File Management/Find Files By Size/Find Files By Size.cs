...
namespace Ayehu.Sdk.ActivityCreation
{
	public class CustomActivity : IActivity
	{
		...
		{
			if(filterMode == "Greater than")
			{
				filterMode = "gt";
			}
			else if(filterMode == "Less than")
			{
				filterMode = "lt";
			}

			ScriptCode = "Get-ChildItem " + searchPath;
			...
			try
			{
				...
				if(HostName.ToLower().Equals("localhost") || HostName.ToLower().Equals("127.0.0.1"))
				{
					...
					{
						throw new Exception("Path is either non-existent or not readable.");
					}
				}
				else
				{
